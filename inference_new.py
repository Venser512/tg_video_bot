...
import signal
import pickle
from config import conf_checkpoint_path

parser = argparse.ArgumentParser(description='Inference code to lip-sync videos in the wild using Wav2Lip models')

parser.add_argument('--reload', type=str, 
					help='Name of the app', required=False, default='wav2lip_api:app')


parser.add_argument('--checkpoint_path', type=str, 
					help='Name of saved checkpoint to load weights from', required=False, default=conf_checkpoint_path)
...
def face_detect_with_cache(images, args, input_video_fname, input_file_size):
    ...
    if not Path(cache_path).exists():
        ...
        print("***")
        print("There's a bug currently (memory leak), please restart 'WAW2LIP' after Face detection process is finished. It is needed just once for each new video.")
        print("***")
        ...
        print("Please restart 'WAV2LIP' (this window) now.")
        kill_flask_server()    
...
def wav2lip_main(face, audio_param, outfile):
   
    global args 
    args.face = face
    args.outfile = outfile
    args.audio = audio_param   
    ...
    if (not len(wav)):
        print("out_"+str(chunk)+".wav len "+str(len(wav))+" is bad, exiting")
        return
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
        ...
        if i == 0:
            ...
            out = cv2.VideoWriter('temp/result_'+str(chunk)+'_tmp.mp4', cv2.VideoWriter_fourcc(*'avc1'), fps, (frame_w, frame_h)) # 0.00s test mp2v avc1
        ...
        try:
            ...
            out.release()
            os.replace('temp/result_'+str(chunk)+'_tmp.mp4', 'temp/result_'+str(chunk)+'.mp4')
            torch.cuda.empty_cache()
            return -1
    ...
    if "out" in locals():
        out.release()
    if (os.path.isfile('temp/result_'+str(chunk)+'_tmp.mp4')):
        os.replace('temp/result_'+str(chunk)+'_tmp.mp4', 'temp/result_'+str(chunk)+'.mp4')
    ...
    #command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio, 'temp/result_'+str(chunk)+'.avi', args.outfile) # mp4:0.30-0.23s.  avi: 0.12-0.13s
    #command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 -map 1:v -map 0:a -codec copy {}'.format(args.audio, 'temp/result_'+str(chunk)+'.ts', args.outfile) # mp4 copy: 0.04s, plays in chrome on win11, todo: test in android
    command = 'ffmpeg -y -i {} -i {} -g 60 -hls_time 2 -hls_list_size 0 -start_number {} {}'.format(args.audio, 'temp/result_'+str(chunk)+'.mp4', chunk, args.outfile) # mp4 copy: 0.04s, plays in chrome on win11, todo: test in android
